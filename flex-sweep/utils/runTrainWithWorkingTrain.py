...
def trainModel(argsDict):
        ...
        trainSplit = int(argsDict["fvSplit"].split(" ")[1])/100
        ...
        history = model.fit(datagen.flow(X_train, Y_train, batch_size=   1   ), 
                                      steps_per_epoch=len(X_train)/   1   , epochs=100, verbose=1, 
                                      callbacks=callbacks_list, 
                                      validation_data=validation_gen.flow(X_valid,Y_valid, batch_size=   1   ), 
                                      validation_steps=len(X_valid)/   1   )
        val_score = model.evaluate(validation_gen.flow(X_valid, Y_valid, batch_size=   1   ), batch_size=   1   , steps=len(Y_valid)/   1   )
        test_score = model.evaluate(test_gen.flow(X_test, Y_test, batch_size=   1   ), batch_size=   1   , steps=len(Y_test)/   1   )
        
        train_score = model.evaluate(datagen.flow(X_train, Y_train, batch_size=   1   ), batch_size=   1   , steps=len(Y_train)/   1   )
        print(f"Training and testing model took {time.time()-start} seconds")
        
        with open(f"{outputModel}_history.csv",'w') as csvfile:
                writer=csv.writer(csvfile)
                header=history.history.keys()
                rows=zip(*[history.history[h] for h in header])
...
def main(argsDict):
        if argsDict['continue']:
                weightsPath = f"{argsDict['outputDir']}/{argsDict['outputDir']}Model/saved_model.pb"
