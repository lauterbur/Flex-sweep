...
def trainModel(argsDict):
        ...
        val_score = model.evaluate(validation_gen.flow(X_valid, Y_valid, batch_size=32), batch_size=32, steps=len(Y_valid)/32)
        test_score = model.evaluate(test_gen.flow(X_test, Y_test, batch_size=32), batch_size=32, steps=len(Y_test)/32)
        
        train_score = model.evaluate(datagen.flow(X_train, Y_train, batch_size=32), batch_size=32, steps=len(Y_train)/32)
        ...
        with open(f"{modelPath}/testPredictions/{type}.preds",'w') as outFile:
                if numCols == 1155:
                        ...
                        for row in range(0,len(preds)):
                                outFile.write(f"{classDict[predictions[row]]}\t{preds[row][0]}\t{preds[row][1]}\n")
                elif numCols == 1155+4:
