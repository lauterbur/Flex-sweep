...
def parse_arguments(commandline):
        ...
        parser.add_argument('--predLoc', help='optional, the path to the directory containing the neutral and sweep predictions for which to calculate and plot accuracy\nif not used, assumes predictions exist in outputDir/modelDir/modelPredictions')
...
def plotAcc(argsDict):
        rCommand = f"Rscript --vanilla utils/trainPlots.R {argsDict['predLoc']}/neutral.preds {argsDict['predLoc']}/sweep.preds {argsDict['outputDir']}"
        rCommandRun = subprocess.Popen(rCommand.split(),stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        rOutput,rError = rCommandRun.communicate()
...
def main(commandline):
        ...
        argsDict = parse_arguments(commandline) # needs outputDir + (optional) predLoc (train) or classifyname (classify)
        print(argsDict)

        os.makedirs(f"{argsDict['outputDir']}/plots", exist_ok=True)

        if argsDict['mode'] == "train":
                if not argsDict['predLoc']:
                        argsDict['predLoc'] = f"{argsDict['outputDir']}/{argsDict['outputDir']}Model/{argsDict['outputDir']}Predictions"
                plotAcc(argsDict)
