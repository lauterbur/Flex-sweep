...
import csv
from utils import runTrainSmall
...
def splitFV(argsDict, type):
        ...
        allFVs = []
        if os.path.exists(f"{argsDict['fvLoc']}/{type}.fv") and os.path.getsize(f"{argsDict['fvLoc']}/{type}.fv") > 0:
                lineCount = 0
                with open(f"{argsDict['fvLoc']}/{type}.fv", "rb") as fp:
                        c_generator = _count_generator(fp.raw.read)
                        lineCount = sum(buffer.count(b'\n') for buffer in c_generator)
                        lineCount += 1
                if lineCount < 2:
                        print(f"Specified feature vector {argsDict['fvLoc']}/{type}.fv has no data")
                        sys.exit(1)
                else:
                ...
                print(f"Feature vector {argsDict['fvLoc']}/{type}.fv does not exist or is empty")
                sys.exit(1)
